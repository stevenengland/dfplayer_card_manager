...
class TestAllocationUnitSizeDetectionMacOs:
    def test_check_allocation_unit_size_macos_exists_and_has_correct_allocation_unit_size(
        self,
        when,
    ):
        # GIVEN
        ...
        mock_subprocess_run = MagicMock(
            stdout="st_dev=16777232 st_ino=2 st_rdev=0 st_size=32768 st_blksize=1048576 st_blocks=64 st_flags=65536",  # noqa: E501
            returncode=0,
        )
        when(subprocess).run(["stat", "-s", "/Volumes/sbstick"], ...).thenReturn(
            mock_subprocess_run,
        )
    ...
    def test_check_allocation_unit_size_macos_exists_and_has_correct_allocation_unit_size_using_device_path(
        self,
        when,
    ):
        # GIVEN
        ...
        when(builtins).open("/dev/disk4", ...).thenReturn(mock_file)
        # WHEN
        has_correct_allocation_unit_size = check_has_correct_allocation_unit_size(
            "/dev/disk4",
        )
    ...
    def test_check_allocation_unit_size_macos_exists_and_has_incorrect_allocation_unit_size(
        self,
        when,
    ):
        # GIVEN
        ...
        mock_subprocess_run = MagicMock(
            stdout="st_dev=16777232 st_ino=2 st_rdev=0 st_size=16386 st_blksize=1048576 st_blocks=64 st_flags=65536",  # noqa: E501
            returncode=0,
        )
        when(subprocess).run(...).thenReturn(mock_subprocess_run)
